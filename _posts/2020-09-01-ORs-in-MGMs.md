...
$$
\text{Odds}_{X_2=1} = \frac{P(X_1=1|X_2=1)}{P(X_1=0|X_2=1)} \neq
\frac{P(X_1=1|X_2=0)}{P(X_1=0|X_2=0)} = \text{Odds}_{X_2=0}
.
$$
...
The first set of parameters in `coefs[[1]]` models $P(X_5 = 0| \dots)$ and the second set of parameters in `coefs[[2]]` models $P(X_5 = 1| \dots)$.
...
Now, back to computing those probabilities in the enumerator. We would like to compute $P(X_5=1|X_1=0)$ and $P(X_5=0|X_1=0)$, however we see that the probability of $P(X_5)$ not only depends on $X_1$ but also on all other variables (none of the parameters are zero). We therefore need to fix all variables to some value in order to obtain a conditional probability. That is, we actually have to write the conditional probabilities as $P(X_5=1|X_1, X_2, X_3, X_4, X_6, X_7)$ and $P(X_5=0|X_1, X_2, X_3, X_4, X_6, X_7)$. Here we will fix all other variables to 0, but we will see later that it does not matter for our OR calculation to which value we set all these variables, as long as we choose the same values in all of the four probabilities.

The probability of $P(X_5=0|\dots)$ is calculated by dividing the potential for this category by the sum of all (here two) potentials:

$$
P(X_5=0|\dots) = \frac{
\text{Potential}(X_5=0|\dots)
}{
\text{Potential}(X_5=0|\dots) + \text{Potential}(X_5=1|\dots)
}
$$
...
$$
\text{Potential}(X_5=1|X_1=0, X_2=0, \dots, X_7=0) =
\exp \{
...
We calculated that the probability of $P(X_5=0|\dots)$ (supervised housing) is $\approx 0.78$ and the probability of $P(X_5=0|\dots)$ (unsupervised housing) is $\approx 0.22$.
...
Similar to above, `coefs[[1]][1,1]` contains the intercept and `coefs[[1]][2,1]` contains the parameter associated with predictor $X_1$ for probability $P(X_5 = 0| \dots)$. `coefs[[2]]` contains the corresponding parameters for probability $P(X_5 = 1| \dots)$.
...
=
\frac{
\frac{P(X_1=1|X_2=1,X_3=x_3)}{P(X_1=0|X_2=1,X_3=x_3)}
}{
\frac{P(X_1=1|X_2=0,X_3=x_3)}{P(X_1=0|X_2=0,X_3=x_3)}
}
.
$$
...
In the first step we fixed $X_1=1$ in the numerator and $X_1=0$ in the denominator and simplified. The parameter $\beta_{21}$ refers to the coefficient associated with $X_2$ in the equation modeling $P(X_1=1|\dots)$, while $\beta_{20}$ refers to the coefficient associated with $X_2$ in the equation modeling $P(X_1=0|\dots)$. 
...
$$
P(X_5=1|X_1=1, \dots) - P(X_5=0|X_1=0, \dots)
.
$$
